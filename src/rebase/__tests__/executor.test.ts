...
import type { RebaseIntentNode } from '../../protocol';
...
describe('RebaseQueueExecutor', () => {
  ...
  it(
    'rebases child branches after amending a parent commit message',
    async () => {
      const repoDir = mkdtempSync(join(tmpdir(), 'teapot-vscode-rebase-executor-'));
      tempDirs.push(repoDir);
      ...
      git(repoDir, ['checkout', '--quiet', '-b', 'feature', mainHeadSha]);
      commitFile(repoDir, 'feature.txt', 'feature before amend');
      const originalFeatureHeadSha = revParse(repoDir, 'HEAD');
      ...
      const before = await GitStackBuilder.build(repoDir);
      const fixupBefore = before.branches.find((branch) => branch.ref === 'fixup');
      if (!fixupBefore) {
        throw new Error('Expected fixup branch');
      }

      const subtree: RebaseIntentNode = {
        branchRef: fixupBefore.ref,
        headSha: fixupBefore.headSha,
        baseSha: fixupBefore.baseSha,
        children: [],
      };

      const gitClient = await GitClient.open(repoDir);
      if (!gitClient) {
        throw new Error('Expected git client');
      }
      await gitClient.checkout('feature');
      await gitClient.amendCommitMessage('feature after amend');
      const amendedFeatureHeadSha = await gitClient.revParse('HEAD');

      const store = new OperationQueueStore(repoDir);
      const queue = QueueBuilderUtils.fromSubtrees(
        [subtree],
        { kind: 'sha', sha: amendedFeatureHeadSha },
        {
          repoRoot: repoDir,
          originalBranchRef: 'feature',
          label: 'test',
        }
      );
      await store.save(queue);
      ...
      expect(feature?.headSha).toBe(amendedFeatureHeadSha);
      expect(feature?.headSha).not.toBe(originalFeatureHeadSha);
      expect(fixup?.headSha).not.toBe(originalFixupHeadSha);
      expect(fixup?.parentRef).toBe('feature');
      expect(fixup?.baseSha).toBe(amendedFeatureHeadSha);
    },
    15_000
  );
});
