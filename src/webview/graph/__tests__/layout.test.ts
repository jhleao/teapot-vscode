...
describe('layoutRows', () => {
  ...
  it('anchors direct child branches around the matching trunk commit instead of hoisting all of them above trunk', () => {
    const state: StackState = {
      branches: [
        {
          ref: 'main',
    ...
    expect(rows.filter((row) => row.branchName === 'main' && row.kind === 'commit')).toHaveLength(2);
  });
